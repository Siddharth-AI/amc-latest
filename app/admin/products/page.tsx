...
export default function ProductsPage() {
  ...
  const handleHardDelete = async (id: string) => {
    if (
      !confirm(
        "Are you sure you want to permanently delete this product? This action cannot be undone and will delete all images from Cloudinary.",
      )
    ) {
      return;
    }
  ...
  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    ...
    try {
      await dispatch(
        toggleProductStatus({ id, isActive: !currentStatus }),
      ).unwrap();
      toast.success(
                  ...
                  {productsList.map((product) => (
                    <tr key={product.id} className="hover:bg-gray-50">
                      <td className="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap">
                        <div className="text-xs sm:text-sm font-medium text-navy-900">
                          {product.name}
                        </div>
                      </td>
                      ...
                      <td className="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap">
                        <button
                          ...
                          disabled={
                            togglingId === product.id || product.is_deleted
                          }
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {selectedProduct.images.map((img, idx) => (
                      <div
                        ...
                        <Image
                          src={
                            getProductImageUrl(img.base_url, img.img_name) ||
                            "/placeholder.png"
                          }
              ...
              <div>
                ...
                <p className="text-sm text-gray-900 mt-1">
                  {selectedProduct.is_warranty
                    ? selectedProduct.warranty_period || "Yes"
                    : "No"}
                </p>
              </div>
